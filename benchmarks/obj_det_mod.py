...
def main():
    ...
    for batch in tqdm(data_loader):
        ...
        # Taking bounding box where the gaze lies
        bbx_pred = None
        for b in bbx_preds:
        ...
        # If the gaze is inside no predicted bounding box, the closest one is chosen
        if bbx_pred is None:
            min_dist = float('inf')
            for b in bbx_preds:
                if box_dist(b, gaze_p) < min_dist:
                    bbx_pred = b
                    min_dist = box_dist(b, gaze_p)
